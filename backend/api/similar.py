...
The seed is a tile (or the after-date tile under a change candidate). Its own embedding, taken from the FAISS index, is
the query, so no model runs and the answer is the tiles that look most like it, in every scene of the archive.
"""
...
class SimilarRequest(BaseModel):
    """Exactly one seed: a tile, or a change candidate (its after-date tile is used)."""
    ...
    def _one_seed(self) -> "SimilarRequest":
        if (self.tile_id is None) == (self.candidate_id is None):
            raise ValueError("Give exactly one of tile_id or candidate_id")
        return self
...
def find_similar(payload: SimilarRequest) -> SimilarResponse:
    ...
    instrumentation.record_query(instrumentation.QUERY_SIMILAR, time.perf_counter() - started)
    return SimilarResponse(
        ...
        label=f"Similar to {seed_meta['tile_id']}",
